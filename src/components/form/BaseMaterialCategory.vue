<template>
  <div>
          ...
          <thead>
            <tr>
              <th style="min-width: 300px; border: 1px solid #bbb">
                <div style="margin-top: 8px; font-weight: 100">Mã nhóm</div>
                <div style="display: flex; min-width: 100%; margin: 8px 0">
                  <BaseFilterSearch type="string" field="categoryCode" @changeFilter="changeFilter" />
                </div>
              </th>
              <th style="min-width: 300px; border: 1px solid #bbb">
                <div style="margin-top: 8px; font-weight: 100">Tên nhóm</div>
                <div style="display: flex; min-width: 100%; margin: 8px 0">
    ...
    <TheFormMaterialCategory v-if="showForm" @onClose="showForm = false" :id="id" :type="type"
      @saveDataSucces="saveDataSucces" @saveDataFail="saveDataFail" @saveDataAndAddSuccess="saveDataAndAddSuccess"
      @saveDataAndAddFail="saveDataAndAddFail" @getNewForm="getNewForm"></TheFormMaterialCategory>
...
export default {
  ...
  data() {
    return {
      titleF: Resource.TitleContent.MaterialCategory,
      dataTable: [],
      dataFilter: [
        {
          field: "status",
  ...
  methods: {
    ...
    getPaging(pageSize, pageChoice, sort, dataFilter) {
      try {
        ...
        const data = callAPI(
          "POST",
          materialCategoryService.filterAndPaging(pageSize, pageChoice, sort),
          dataFilter
        );
        data.then((res) => {
      ...
      try {
        if (this.dataSelceted && this.popup.callbackFunc) {
          ...
          this.popup.callbackFunc.callbackWarningDeleteYes = () => {
            // Call API xóa
            fetchAPI(
              materialCategoryService.deleteRecord(this.dataSelceted.categoryID),
              ...
              (res) => {
                ...
                if (res == enumMISA.response.Success) {
                  ...
                  this.customPopup(true, `${Resource.ToastMessage.NotDelete.TextCategory} <${this.dataSelceted.categoryCode} - ${this.dataSelceted.categoryName}> ${Resource.ToastMessage.NotDelete.AfterText}`, Resource.VUE_APP_POPUP.ERROR);
                  // this.deleteFail();
                } else {
          ...
          this.customPopup(
            true,
            `${Resource.ERROR_VALIDATE_FE.RequireChoiceMaterialCategory}`,
